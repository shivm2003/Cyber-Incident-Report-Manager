...
def process_cve_automation(db: Session, cve_db_id: int):
    ...
    if res["score"] >= 70:
        ...
        # 3. Create Jira Ticket & Push History
        try:
...
def run_automation_cycle(timeframe_nvd="today", timeframe_rss="today", run_nvd=True, run_rss=True):
    ...
    try:
        ...
        if run_nvd:
            ...
            if new_cve_ids:
                current_status_message = f"Processing {len(new_cve_ids)} new CVEs..."
                print(f"[*] Scheduler: AI processing and scanning {len(new_cve_ids)} new CVEs...")
                for cve_db_id in new_cve_ids:
        ...
        if run_rss:
            ...
            new_incident_ids = rss_result.get("new_ids", [])
            if new_incident_ids:
                current_status_message = f"Analyzing impact for {len(new_incident_ids)} new incidents..."
                print(f"[*] Scheduler: Analyzing impact for {len(new_incident_ids)} new incidents...")
                for inc_id in new_incident_ids:
